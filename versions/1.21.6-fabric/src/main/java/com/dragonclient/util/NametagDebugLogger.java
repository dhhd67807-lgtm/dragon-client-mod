...
public final class NametagDebugLogger {
    ...
    private static synchronized void ensureWriter() {
        ...
        try {
            ...
            Path runDir = (client != null && client.runDirectory != null)
                    ...
                    : Paths.get(".");
            logPath = runDir.resolve("logs").resolve("dragonclient-nametag-debug.log");
            ...
            log("=== Nametag Debug Logger Started ===");
            log("Log file location: " + logPath.toAbsolutePath());
        } catch (IOException e) {
            initFailed = true;
            System.err.println("Failed to create nametag debug log: " + e.getMessage());
        }
    }
