...
public class CosmeticsScreen extends Screen {
    ...
    private float playerRotation = 0; // Start facing front (0 = forward in entity coords)
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        // Draw player model directly (INSIDE scaled matrix)
        int playerModelX = previewX + previewWidth / 2;
        int playerModelY = previewY + previewHeight - 60;
        int modelSize = 100;
        
        // Render 3D player model with rotation using Vector3f + Quaternionf signature
        if (client.player != null) {
            // Stop player animation (limb swing)
            float prevLimbDistance = client.player.limbAnimator.getSpeed();
            client.player.limbAnimator.setSpeed(0);
            
            // Use the fixed rotation signature: (context, x1, y1, x2, y2, size, pos, rotation, headRotation, entity)
            org.joml.Vector3f pos = new org.joml.Vector3f(0, 0, 0);
            org.joml.Quaternionf rotation = new org.joml.Quaternionf().rotationY((float)Math.toRadians(180.0f + playerRotation));
            
            net.minecraft.client.gui.screen.ingame.InventoryScreen.drawEntity(
                context,
                playerModelX - 50, playerModelY - 80,  // x1, y1 (bounding box top-left)
                playerModelX + 50, playerModelY + 80,  // x2, y2 (bounding box bottom-right)
                (float)modelSize,
                pos,
                rotation,
                null,  // head rotation
            ...
            client.player.limbAnimator.setSpeed(prevLimbDistance);
        }
        ...
        // In 1.21.6, enableScissor uses logical scaled GUI coordinates
        ...
        for (int i = startIndex; i < endIndex; i++) { // Show items for current page
            ...
            if (selectedTab.equals("CAPES") && dummyPlayer != null) {
                ...
                org.joml.Vector3f capePos = new org.joml.Vector3f(0, 0, 0);
                org.joml.Quaternionf capeRotation = new org.joml.Quaternionf().rotationY((float)Math.toRadians(135.0f));
                
                net.minecraft.client.gui.screen.ingame.InventoryScreen.drawEntity(
                    context,
                    modelX - 25, modelY - 50,  // x1, y1
                    modelX + 25, modelY + 50,  // x2, y2
                    (float)previewModelSize,
                    capePos,
                    capeRotation,
                    null,
                    dummyPlayer
    ...
    // 1.21.10: mouseReleased signature changed - remove @Override
    ...
    // 1.21.10: mouseDragged signature changed - remove @Override
