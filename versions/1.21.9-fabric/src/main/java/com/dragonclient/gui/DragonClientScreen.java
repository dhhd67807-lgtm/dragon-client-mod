...
public class DragonClientScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int scissorMinX = fixedGuiLeft;
        int scissorMinY = fixedGuiTop + 105;
        int scissorMaxX = fixedGuiLeft + GUI_WIDTH;
        int scissorMaxY = fixedGuiTop + GUI_HEIGHT;
        ...
        for (Module module : modules) {
            if (module.getCategory() == selectedCategory) {
                ...
                if (cardY + CARD_HEIGHT > fixedGuiTop + 105 && cardY < fixedGuiTop + GUI_HEIGHT) {
                    ...
                    boolean isOutlinesCard = "Outlines".equalsIgnoreCase(normalizedModuleName);
                    int cardBackgroundColor = isTierTaggerCard
                        ? 0xFF3A2C12
                        : (isFreelookCard ? 0xFF1A2940 : (isOutlinesCard ? 0xFF173824 : 0xFF252220));
                    ...
                    } else if (isOutlinesCard) {
                        drawTextureWithColor(context, CARD_TEXTURE, cardX, cardY, CARD_WIDTH, CARD_HEIGHT, 0x664BD87F);
                    } else if (cardIndex < 3) {
